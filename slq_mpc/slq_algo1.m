...
if nargin == 0
    if modelParams.gen_traj
        [nom_traj.x, nom_traj.u]=generateTraj(modelParams, simple_pend, modelParams.nom);
        ...
        if modelParams.traj_track
            ...
            des_traj.u = zeros(6,modelParams.N);
            save('desired.mat','des_traj');
        end
    else
...
while max_iter < 100
    ...
    for time_iter = 1:modelParams.N-1
        ...
        % TODO change the tilthex inputs
        [~, nom_traj.x(:,time_iter+1)] = tilthex_dynamics(nom_traj.x(:,time_iter),...
...
%     x_diff(1,:) = wrapToPi(x_diff(1,:));  % SUS
    u_diff = nom_traj.u-des_traj.u;
    
    % chris and andrew changed
    p1 = zeros(12, 12, modelParams.N);  % corresponds to P in the psuedocode
    p1(:,:,modelParams.N) = 2*modelParams.Qf;
    p2=zeros(12, modelParams.N);  % corresponds to bold p in the psuedocode
    ...
    K = zeros(6, 12, modelParams.N-1);  % changed
    l = zeros(6, modelParams.N-1);
    ...
    for ricatti_iter = modelParams.N-1:-1:1
        ...
        % [12x12] + [12x12][12x12][12x12] + [12x6][6x6][6x12] +
        % [12x6][6x12] + [12x6][6x12]
