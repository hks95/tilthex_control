...
for i = 2:size(x_next,2)
    ...
    state.roll =        x_next(7, i-1);
    state.pitch =       x_next(8, i-1);
...
%     state.R
    ...
    new_attitude  = [state.roll;state.pitch;state.yaw] + dot_attitude*dt;

    state.pitch = asin(-state.R(3, 1));
    state.yaw   = atan2(state.R(2, 1), state.R(1, 1));
    state.roll  = atan2(state.R(3, 2), state.R(3, 3));
    
    if(abs(sum(new_attitude - [state.roll;state.pitch;state.yaw])) > 0.01)
    %if (new_attitude(1,1) ~= state.roll || new_attitude(2,1) ~= state.pitch ||  new_attitude(3,1)~= state.yaw)
        disp("RPY DOESN'T MATCH!");
    end
